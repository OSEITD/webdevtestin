...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ❌ REMOVED: loadTrips(); - Don't load trips on page load
    setupFeeCalculation();
    setupTripSelection(); // Setup trip selection handlers
    setupAutofill(); // Setup autofill functionality
    setupDestinationOutletChangeHandler(); // Reload trips when destination changes
});
...
function initializePhotoUpload() {
    ...
    if (!uploadZone || !fileInput || !photoPreview) {
        console.error('Photo upload elements missing');
        return;
    }

    // Click on upload zone opens file selector for multiple files
    ...
    // Camera button opens camera for single photo
    if (cameraBtn) {
    ...
    // Function to open camera modal and capture photo
        ...
        captureBtn.addEventListener('click', () => {
            ...
            canvas.toBlob(blob => {
                if (blob) {
                    ...
                    // Add to selectedPhotos if not duplicate
                    if (!selectedPhotos.some(f => f.name === file.name && f.lastModified === file.lastModified)) {
...
function handleFileSelection(files) {
    ...
    for (let file of files) {
        if (!file.type.match(/^image\/(jpeg|png|gif)$/)) {
            showError('Only JPG, PNG, GIF images are allowed.');
            continue;
        }
        ...
        // Prevent duplicates by name and lastModified
        if (!selectedPhotos.some(f => f.name === file.name && f.lastModified === file.lastModified)) {
            selectedPhotos.push(file);
            addedCount++;
        }
...
function renderPhotoPreviews() {
    ...
    selectedPhotos.forEach(file => {
        ...
        reader.onload = function(e) {
            ...
            div.innerHTML = `
                <img src="${e.target.result}" alt="Photo" style="max-width:80px;max-height:80px;object-fit:cover;" />
                <button type="button" class="remove-photo-btn" title="Remove" aria-label="Remove photo" data-filename="${file.name}" data-lastmodified="${file.lastModified}">
            ...
            removeBtn.addEventListener('click', () => {
                removePhoto(file.name, file.lastModified);
            });
        };
...
function removePhoto(name, lastModified) {
    selectedPhotos = selectedPhotos.filter(f => !(f.name === name && f.lastModified === lastModified));
    renderPhotoPreviews();
    updatePhotoUploadUI();
}
...
function updatePhotoUploadUI() {
    ...
    if (uploadText) {
        uploadText.textContent = remaining > 0
            ? `Click here or drag and drop images (${remaining} left)`
            : 'Maximum photos uploaded';
    }
    uploadZone.classList.toggle('disabled', remaining <= 0);
    uploadZone.style.pointerEvents = remaining <= 0 ? 'none' : 'auto';
}
...
async function loadTrips() {
    ...
    try {
        ...
        if (destinationOutlet) {
            ...
            // TEMPORARY: Add debug flag to bypass filter
            // url += `&debug_no_filter=1`; // Uncomment to see ALL trips regardless of destination
        }
        
        console.log('  - Fetching from URL:', url);
        console.log('  - Destination outlet ID being sent:', destinationOutlet);
        console.log('  - URL encoded version:', encodeURIComponent(destinationOutlet));
...
// Setup destination outlet change handler to reload trips
function setupDestinationOutletChangeHandler() {
    ...
    console.log('  - tripInstructionMessage:', tripInstructionMessage ? '✅ Found' : '❌ Not found');
    console.log('  - tripAssignmentContent:', tripAssignmentContent ? '✅ Found' : '❌ Not found');
...
function setupFeeCalculation() {
    ...
    // Add event listeners for automatic fee calculation
    [weightInput, valueInput, deliveryOption, insuranceInput].forEach(input => {
        if (input) {
...
function calculateDeliveryFee() {
    console.log('🔥 calculateDeliveryFee function called - NEW VERSION');
    // New manual fee system - just update the display elements based on user input
...
async function handleParcelSubmission(event) {
    ...
    try {
        ...
        formData.append('codAmount', document.getElementById('codAmount')?.value || '0');
            ...
            // Fix payment method validation to allow 'visa'
            const allowedPaymentMethods = ['cash', 'mobile_money', 'visa', 'cod'];
...
function validateForm() {
    const required = [
        ...
        // Made optional: deliveryFee (can be calculated), paymentMethod (can default), deliveryOption (has default)
        // tripId intentionally omitted to make it optional
    ];
    let isValid = true;
    ...
    // ✅ NEW: Validate trip-destination compatibility
    const tripSelect = document.getElementById('tripId');
    const destinationSelect = document.getElementById('destinationOutletId');
    ...
    if (tripSelect && tripSelect.value && destinationSelect && destinationSelect.value) {
        ...
        // Outlet select text may include location suffix "OutletZero - Ndeke Inn"; use only the name part.
        ...
        // Check if trip includes this destination (case-insensitive, partial-match)
        const intermediateStops = JSON.parse(selectedOption.getAttribute('data-intermediate-stops') || '[]');
        const allStops = [
...
function showError(message) {
    ...
    if (!errorDiv) {
        ...
        document.querySelector('.content-container').insertBefore(errorDiv, document.querySelector('form'));
    }
    
    errorDiv.innerHTML = `
        <i class="fas fa-exclamation-triangle"></i>
        <span>${message}</span>
        <button type="button" onclick="this.parentElement.remove()" class="close-btn">
            <i class="fas fa-times"></i>
        </button>
    `;
            ...
            ">
                ...
                <div style="padding: 20px;">
                    ...
                    ">
                        ...
                        <div style="margin-bottom: 8px;">
                            <strong>Trip Assigned:</strong> <span style="color: ${result?.trip_assigned ? '#28a745' : '#dc3545'}; font-weight: bold;">${result?.trip_assigned ? '✓ Yes' : '✗ Failed'}</span>
                        </div>
                        ...
                        ${result?.barcode_error ? `<div style='color:#dc3545; margin-bottom:8px;'><strong>Barcode Error:</strong> ${result.barcode_error}</div>` : ''}
                        <div style="margin-bottom: 8px;">
                ...
                ">
                    ...
                    ">
                        ...
                        <button onclick="document.querySelector('.success-modal-overlay').remove(); window.location.reload();" style="
                            ...
                            min-width: 140px;
                            max-width: 160px;
                        " onmouseover="this.style.background='#4A1C40'" onmouseout="this.style.background='#2E0D2A'">
...
console.log('🔥 Parcel Registration JS Loaded Successfully - VERSION 2.0 - ' + new Date().toISOString());

// OPTIMIZED: Autofill customer data by NRC or Phone
