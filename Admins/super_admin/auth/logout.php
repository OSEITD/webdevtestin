...
// Track logout in user_sessions table before clearing session
try {
    $userId = $_SESSION['user_id'] ?? null;
    $sessionId = $_SESSION['session_id'] ?? null;
    
    if ($userId && $sessionId) {
        // Update logout_time for the active session
        $currentTime = date('Y-m-d H:i:s') . '+00';
        $updateResult = callSupabaseWithServiceKey(
        ...
        if ($updateResult) {
            error_log("Session logout recorded: {$sessionId} at {$currentTime}");
        } else {
            error_log("Warning: Failed to record session logout for {$sessionId}");
        }
    }
} catch (Exception $e) {
