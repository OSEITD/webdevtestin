...
if ($userId) {
    try {
        ...
        // Fetch user details: prefer the base `users` table, but fall back to `all_users` view if missing
        $userResp = null;
        try {
            $userResp = $client->get("users", "id=eq.{$userId}&select=*");
        } catch (Exception $e) {
            error_log('Sidebar: users table fetch failed, trying all_users view: ' . $e->getMessage());
            try {
                $userResp = $client->get("all_users", "id=eq.{$userId}&select=*");
                error_log('Sidebar: fetched user from all_users view');
            } catch (Exception $e2) {
                error_log('Sidebar: all_users view fetch also failed: ' . $e2->getMessage());
                throw $e2;
            }
        }
