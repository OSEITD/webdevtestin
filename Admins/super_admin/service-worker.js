...
self.addEventListener('push', (event) => {
    ...
    if (!event.data) {
        console.warn('[Admin SW] Push event has no data');
        return;
    }
    ...
    try {
        ...
        console.log('[Admin SW] Showing notification with title:', data.title);

        event.waitUntil(
            self.registration.showNotification(data.title || 'Admin Notification', options)
                .then(() => console.log('[Admin SW] Notification shown successfully'))
                .catch(err => console.error('[Admin SW] Failed to show notification:', err))
        );
    } catch (error) {
        console.error('[Admin SW] Error processing push:', error);
...
self.addEventListener('notificationclick', (event) => {
    ...
    const urlToOpen = event.notification.data?.url || `${baseUrl}/Admins/super_admin/index.php`;

    event.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
            for (const client of clientList) {
                if (client.url.includes('/Admins/super_admin/') && 'focus' in client) {
...
self.addEventListener('pushsubscriptionchange', (event) => {
    console.log('[Admin SW] Push subscription expired or changed');
    console.log('[Admin SW] Old subscription:', event.oldSubscription ? 'exists' : 'none');
    console.log('[Admin SW] New subscription:', event.newSubscription ? 'exists' : 'none');
        ...
        Promise.resolve().then(() => {
            if (event.newSubscription) {
                ...
                if (event.oldSubscription && event.oldSubscription.endpoint) {
                    // Could add logic to notify server about old subscription removal
                }

                // Save new subscription to server
                return fetch('api/push/save_subscription.php', {
                    method: 'POST',
                    ...
                    .then(result => {
                        console.log('[Admin SW] New subscription saved:', result);
                    });
            }
        })
    );
...
console.log('[Admin SW] Service Worker loaded - Push notifications ready');
