...
// Normalize status values to canonical lowercase DB values
...
function renderDrivers(driversToShow) {
    ...
    driversTableBody.innerHTML = driversToShow.map(driver => `
        <tr data-id="${driver.id}">
            ...
            <td data-label="Status">${getStatusBadge(driver.status || 'unavailable')}</td>
            <td data-label="Actions">
                <div class="flex space-x-2">
                    <a href="company-view-driver.php?id=${driver.id}" class="action-btn view-btn">
                        View
                    </a>
                </div>
            </td>
