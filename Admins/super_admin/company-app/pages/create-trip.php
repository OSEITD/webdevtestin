...
async function loadInitialData() {
    try {
            ...
            if (Array.isArray(drivers)) {
                drivers.forEach(driver => {
                    if (driver && driver.id) {
                        ...
                        if (!isAvailable) {
                            option.disabled = true;
                            option.textContent += ` - ${driverStatus}`;
                        }
        ...
        async function submitTrip() {
            ...
            try {
                ...
                if (!isSuccess) {
                    ...
                    submitBtn.innerHTML = `<i class="fas fa-check"></i> ${isEditing ? 'Trip Updated' : 'Trip Created'}`;
                }
            }
        }
        ...
        function clearTripForm() {
            // Reset selects
            ['vehicleId', 'driverId', 'originOutlet', 'destinationOutlet', 'intermediateOutlet'].forEach(id => {
            ...
            if (depTime) {
                ...
                depTime.value = defaultDateTime.toISOString().slice(0, 16);
            }

            // Trigger change on origin to reset disabled states
            handleOriginChange();

            // Reset tripData object
            tripData.vehicle = null;
            tripData.originOutlet = null;
            tripData.destinationOutlet = null;
            tripData.stops = [];
    ...
    try {
        ...
        if (parcelId) {
            ...
            if (routeOutlets.length > 1) {
                // all remaining stops = possible destinations
                const destinations = routeOutlets.slice(1).join(',');
