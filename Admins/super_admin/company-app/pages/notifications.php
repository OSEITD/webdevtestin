...
<style>
    ...
    .content-container { max-width: 1200px; margin: 0 auto; width: 100%; padding: 20px; }
    .back-button { 
    ...
    <section class="notifications-shell">
        <header class="notifications-hero">
            <div class="notifications-hero-text">
                <span class="notifications-hero-eyebrow">Company Alerts Centre</span>
                <h1>
    ...
    class NotificationsPage {
        ...
        init() {
            ...
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape') this.hideActionModal();
            });

            document.addEventListener('visibilitychange', () => {
                if (document.hidden) this.stopAutoRefresh();
                else { this.startAutoRefresh(); this.loadNotifications(); }
            });
        ...
        bindEvents() {
            ...
            ['typeFilter', 'dateFilter', 'priorityFilter'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.addEventListener('change', () => { this.currentPage = 1; this.loadNotifications(); });
            });
        }
        ...
        async loadNotifications(silent = false) {
            ...
            try {
                ...
                if (!silent) {
                    ...
                    if (container) {
                        container.innerHTML = `<div style="padding: 40px; text-align: center; color: var(--text-secondary);"><i class="fas fa-spinner fa-spin fa-2x"></i><div style="margin-top: 12px;">Loading notifications...</div></div>`;
                    }
                }
                
                let url = `../../api/notifications.php?action=list&page=${this.currentPage}&limit=${this.limit}&status=${this.currentFilter}`;
                const tf = document.getElementById('typeFilter'); if (tf && tf.value) url += `&type=${encodeURIComponent(tf.value)}`;
                const df = document.getElementById('dateFilter'); if (df && df.value) url += `&date=${encodeURIComponent(df.value)}`;
                const pf = document.getElementById('priorityFilter'); if (pf && pf.value) url += `&priority=${encodeURIComponent(pf.value)}`;
                
                this.currentController = new AbortController();
                const timeoutId = setTimeout(() => { if (this.currentController && !this.currentController.signal.aborted) this.currentController.abort(); }, 15000);
                
                const response = await fetch(url, { signal: this.currentController.signal, headers: { 'Accept': 'application/json', 'Cache-Control': 'no-cache' } });
                ...
                if (data.success) {
                    ...
                    if (data.unread_count > this.lastUnreadCount && this.lastUnreadCount > 0) this.showNewNotificationAlert(data.unread_count - this.lastUnreadCount);
        ...
        renderNotifications() {
            ...
            container.innerHTML = `<div class="notifications-grid">${this.notifications.map((n, i) => this.renderNotificationCard(n, i)).join('')}</div>`;

            setTimeout(() => {
                container.style.opacity = '1';
                container.querySelectorAll('.notification-card').forEach((card, index) => {
        ...
        renderNotificationCard(notification, index = 0) {
            ...
            return `
                <div class="notification-card ${notification.status === 'unread' || notification.is_read === false ? 'unread' : ''} ${priorityClass}"
                     data-id="${notification.id}" style="animation-delay: ${index * 50}ms">
                    <div class="notification-card-header">
                        <div class="notification-card-icon" style="background: ${iconColor};"><i class="${icon}"></i></div>
                        <div class="notification-card-content">
                            <div class="notification-card-top">
                                <h3 class="notification-card-title">${this.escapeHtml(notification.title)}</h3>
                                <div class="notification-card-actions">
                                    <input type="checkbox" onchange="notificationsPage.toggleSelection('${notification.id}')" class="notification-checkbox">
                                    <button class="notification-action-btn dropdown-trigger" onclick="notificationsPage.showActionModal('${notification.id}')" title="Actions"><i class="fas fa-ellipsis-v"></i></button>
                                </div>
                            </div>
                            <p class="notification-card-message">${this.escapeHtml(notification.message)}</p>
                            <div class="notification-card-meta">
                                <div class="notification-meta-left">
                                    <span class="notification-time">${timeAgo}</span><span class="meta-separator">•</span>
                                    <span class="notification-type">${(notification.notification_type || 'system').replace('_', ' ')}</span><span class="meta-separator">•</span>
                                    <span class="notification-priority ${notification.priority || 'medium'}">${(notification.priority || 'medium').toUpperCase()}</span>
                                </div>
        ...
        getNotificationIcon(type) {
            const i = { 'parcel_created': 'fas fa-box', 'parcel_status_change': 'fas fa-box-open', 'delivery_assigned': 'fas fa-truck', 'delivery_completed': 'fas fa-check-circle', 'driver_unavailable': 'fas fa-user-times', 'payment_received': 'fas fa-credit-card', 'urgent_delivery': 'fas fa-exclamation-triangle', 'system_alert': 'fas fa-info-circle', 'customer_inquiry': 'fas fa-question-circle' };
            return i[type] || 'fas fa-bell';
        }

        getNotificationIconColor(type) {
            const c = { 'parcel_created': '#10b981', 'parcel_status_change': '#3b82f6', 'delivery_assigned': '#f59e0b', 'delivery_completed': '#22c55e', 'driver_unavailable': '#ef4444', 'payment_received': '#8b5cf6', 'urgent_delivery': '#f97316', 'system_alert': '#6b7280', 'customer_inquiry': '#06b6d4' };
            return c[type] || '#6b7280';
        }

        toggleSelection(notificationId) {
            if (this.selectedNotifications.has(notificationId)) this.selectedNotifications.delete(notificationId);
            else this.selectedNotifications.add(notificationId);
        ...
        showActionModal(notificationId) {
            const btnContainer = document.getElementById('actionModalButtons');
            const n = this.notifications.find(x => x.id === notificationId);
            if (!n) return;
            let btns = [];
            if (n.status === 'unread' || n.is_read === false) {
                btns.push(`<button class="action-modal-btn mark-read-btn" onclick="notificationsPage.markAsRead('${notificationId}'); notificationsPage.hideActionModal();"><i class="fas fa-check"></i> <span>Mark as Read</span></button>`);
            } else {
                btns.push(`<button class="action-modal-btn mark-unread-btn" onclick="notificationsPage.markAsUnread('${notificationId}'); notificationsPage.hideActionModal();"><i class="fas fa-undo"></i> <span>Mark as Unread</span></button>`);
            }
            btns.push(`
                <button class="action-modal-btn archive-btn" onclick="notificationsPage.archiveNotification('${notificationId}'); notificationsPage.hideActionModal();"><i class="fas fa-archive"></i> <span>Archive</span></button>
                <button class="action-modal-btn delete-btn" onclick="notificationsPage.deleteNotification('${notificationId}'); notificationsPage.hideActionModal();"><i class="fas fa-trash"></i> <span>Delete</span></button>
            `);
            btnContainer.innerHTML = btns.join('');
            document.getElementById('actionModal').classList.add('show');
        }

        hideActionModal() { document.getElementById('actionModal').classList.remove('show'); }

        async markAsRead(id) { await this.updateNotification(id, 'mark_read'); }
        async markAsUnread(id) { await this.updateNotification(id, 'mark_unread'); }
        async archiveNotification(id) { await this.updateNotification(id, 'archive'); }
        ...
        async deleteNotification(notificationId) {
            ...
            try {
                ...
                const data = await response.json();
                if (data.success) { this.showToast('Notification deleted successfully', 'success'); this.loadNotifications(true); }
                else throw new Error(data.error);
            } catch (error) { this.showToast(error.message || 'Failed to delete notification', 'error'); }
        }
        ...
        async updateNotification(notificationId, action) {
            try {
                ...
                const data = await response.json();
                if (data.success) { this.showToast('Notification updated successfully', 'success'); this.loadNotifications(true); }
                else throw new Error(data.error);
            } catch (error) { this.showToast(error.message || 'Failed to update notification', 'error'); }
        }
        ...
        startAutoRefresh() {
            ...
            this.refreshInterval = setInterval(() => { if (this.autoRefreshEnabled && !document.hidden) this.loadNotifications(true); }, 30000);
        }
        ...
        updateAnalytics() {
            this.setTextContent('statVisible', this.notifications.length);
            this.setTextContent('statUnread', this.notifications.filter(n => n.status === 'unread' || n.is_read === false).length);
            this.setTextContent('statHighPriority', this.notifications.filter(n => (n.priority || '').toLowerCase() === 'high').length);
            this.setTextContent('statArchived', this.notifications.filter(n => n.status === 'archived').length);
            this.updateStatusLabel();
            this.setTextContent('statFilterSummary', this.buildFilterSummary());
        }

        buildFilterSummary() {
            const tf = document.getElementById('typeFilter'), df = document.getElementById('dateFilter'), pf = document.getElementById('priorityFilter');
        ...
        updateAutoRefreshIndicator() {
            const btn = document.getElementById('autoRefreshBtn'), lbl = document.getElementById('autoRefreshLabel');
            if (btn && lbl) {
        ...
        setTextContent(id, value) { const el = document.getElementById(id); if (el) el.textContent = value; }
        ...
        async markAllAsRead() {
            try {
                const response = await fetch('../../api/notifications.php?action=mark_all_read', { method: 'POST', headers: { 'X-CSRF-Token': document.querySelector('meta[name=\"csrf-token\"]')?.getAttribute('content') || '' } });
                const data = await response.json();
