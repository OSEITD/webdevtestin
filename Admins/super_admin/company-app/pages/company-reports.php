...
<body class="bg-gray-100 min-h-screen">
        ...
        <main class="main-content">
            ...
            <div class="report-section">
                <div class="section-header">
                    ...
                    <div class="report-summary" style="margin-bottom: 20px;">
                        <div class="summary-card">
                            ...
                            <div class="summary-content">
                                ...
                                <p id="commissionRateValue" class="summary-value">—</p>
                            </div>
                        </div>
                        ...
                        <div class="summary-card">
                            ...
                            <div class="summary-content">
                                ...
                                <p id="totalCommissionValue" class="summary-value">—</p>
                            </div>
                        </div>
                        ...
                        <div class="summary-card">
                            ...
                            <div class="summary-content">
                                ...
                                <p id="totalTransactionsValue" class="summary-value">—</p>
                            </div>
                        </div>
                    </div>
                    
                    <div class="report-table" id="revenueBreakdownContainer">
                        <h3>Revenue Breakdown with Commission</h3>
                    ...
                    <div class="report-table" id="allTransactionsContainer" style="margin-top: 20px;">
                        <h3>Transactions</h3>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // We now lookup the Chart instance dynamically in the fetch response handler
    ...
    async function fetchCompanyReports(params = {}) {
        ...
        const res = await fetch(url, { credentials: 'same-origin' });
                ...
                // Set commission summary values
                setText('commissionRateValue', d.commission_rate != null ? (Number(d.commission_rate).toFixed(2) + '%') : '—');
                setText('totalCommissionValue', d.total_commission != null ? formatCurrencyValue(d.total_commission, d.currency) : '—');
                setText('totalTransactionsValue', Number(d.total_transactions || 0).toLocaleString());
                ...
                if (revenueTableBody) {
                    if (Array.isArray(d.revenue_breakdown) && d.revenue_breakdown.length > 0) {
                            revenueTableBody.innerHTML = d.revenue_breakdown.map(r => {
                                ...
                                const commissionRate = (r.commission_rate != null) ? (Number(r.commission_rate).toFixed(2) + '%') : '—';
                                const commission = (r.commission != null) ? formatCurrencyValue(r.commission, d.currency) : '—';
                                const pct = (r.percent != null) ? (Number(r.percent).toFixed(1) + '%') : '—';
                                return `<tr><td>${svc}</td><td>${deliveries}</td><td>${revenue}</td><td>${avg}</td><td>${commissionRate}</td><td>${commission}</td><td>${pct}</td></tr>`;
                            }).join('');
                        } else {
                        // Fallback: try to build from d.top_outlets or parcels/deliveries if available
                        if (Array.isArray(d.top_outlets) && d.top_outlets.length > 0) {
                            revenueTableBody.innerHTML = d.top_outlets.slice(0,5).map(o => `<tr><td>${o.name||o.outlet_name||o.origin_outlet_id||'Outlet'}</td><td>${o.deliveries||o.count||0}</td><td>${o.revenue!=null?formatCurrencyValue(o.revenue, d.currency || o.currency):'—'}</td><td>—</td><td>—</td><td>—</td><td>—</td></tr>`).join('');
                        } else {
                            revenueTableBody.innerHTML = '<tr><td colspan="7">No data</td></tr>';
                        }
                    }
                }

                // Populate all transactions table (showing up to 10 most recent)
                ...
                if (allTransactionsBody) {
                    if (Array.isArray(d.transactions) && d.transactions.length > 0) {
                        allTransactionsBody.innerHTML = d.transactions.slice(0, 10).map(t => {
                ...
                // Update Deliveries by Day chart
                try {
                    let chart = Chart.getChart ? Chart.getChart('deliveriesByDayChart') : null;
                    if (!chart && Chart.instances) {
                        for (const id in Chart.instances) {
                            if (Chart.instances[id].canvas && Chart.instances[id].canvas.id === 'deliveriesByDayChart') {
                                chart = Chart.instances[id]; break;
                            }
                        }
                    }
                    if (chart && d.deliveries_by_day && Array.isArray(d.deliveries_by_day.labels)) {
                        chart.data.labels = d.deliveries_by_day.labels;
                        if (!chart.data.datasets || chart.data.datasets.length === 0) chart.data.datasets = [{ label: 'Deliveries', data: [], backgroundColor: '#6A2A62' }];
                        chart.data.datasets[0].data = d.deliveries_by_day.data;
                        chart.update();
                    }
                } catch (e) {
                    console.warn('Error updating deliveriesByDayChart:', e);
                }

                // Update Delivery Status Doughnut Chart
                try {
                    let dChart = Chart.getChart ? Chart.getChart('deliveryStatusChart') : null;
                    if (!dChart && Chart.instances) {
                        for (const id in Chart.instances) {
                            if (Chart.instances[id].canvas && Chart.instances[id].canvas.id === 'deliveryStatusChart') {
                                dChart = Chart.instances[id]; break;
                            }
                        }
                    }
                    if (dChart && d.delivery_status_counts) {
                        const statuses = Object.keys(d.delivery_status_counts);
                        const counts = Object.values(d.delivery_status_counts);
                        
                        // Dynamically assign colors based on status name keywords
                        const colors = statuses.map(s => {
                            const l = s.toLowerCase();
                            if (l.includes('deliver') || l.includes('complet') || l.includes('success')) return '#10B981'; // Green
                            if (l.includes('cancel') || l.includes('fail') || l.includes('return')) return '#EF4444'; // Red
                            if (l.includes('delay') || l.includes('hold')) return '#F59E0B'; // Orange
                            if (l.includes('assign')) return '#14B8A6'; // Teal (Unique color for assigned)
                            if (l.includes('progress') || l.includes('transit') || l.includes('dispatch')) return '#3B82F6'; // Blue
                            if (l.includes('pend') || l.includes('new') || l.includes('process')) return '#6A2A62'; // Purple
                            return '#' + Math.floor(Math.random() * 16777215).toString(16); // Default random hex
                        });

                        dChart.data.labels = statuses;
                        if (!dChart.data.datasets || dChart.data.datasets.length === 0) dChart.data.datasets = [{ data: [] }];
                        dChart.data.datasets[0].data = counts;
                        dChart.data.datasets[0].backgroundColor = colors;
                        dChart.update();
                    }
                } catch (e) {
                    console.warn('Error updating deliveryStatusChart:', e);
                }
            } catch (err) {
        ...
        if (generateReportBtn) generateReportBtn.addEventListener('click', async function (e) {
            ...
            try {
                ...
                Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to generate report: ' + (err.message || err), confirmButtonColor: '#2e0d2a' });
            } finally {
                btn.disabled = false;
