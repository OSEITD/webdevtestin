...
<body class="bg-gray-100 min-h-screen">
  ...
  // Update status removed from this page - these elements are intentionally not present
    ...
    async function fetchParcelDetails() {
      ...
      if (!trackNumber) {
        Swal.fire({ icon: 'error', title: 'Error', text: 'No tracking number specified.', confirmButtonColor: '#2e0d2a' });
        return;
      }
      ...
      try {
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: 'Error loading parcel details.', confirmButtonColor: '#2e0d2a' });
      }
    }
    ...
    async function saveDestinationForParcel() {
      ...
      if (!response.ok || result.error) {
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to save destination. Please try again.', confirmButtonColor: '#2e0d2a' });
        return;
      }
      ...
      if (error) {
        console.error(error);
        Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to save destination. Check RLS and permissions.', confirmButtonColor: '#2e0d2a' });
        return;
      }
      ...
      Swal.fire({ icon: 'success', title: 'Saved!', text: 'Destination outlet saved.', confirmButtonColor: '#2e0d2a', timer: 2500, timerProgressBar: true });
