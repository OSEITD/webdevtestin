...
try {
    ...
    try {
        ...
        $outletManagerId = $outletRecord[0]['manager_id'] ?? $_SESSION['user_id'] ?? null;
        if (empty($outletManagerId)) {
            error_log("manager_id not found in outlet record, and no user_id in session. Setting to null.");
        }
    } catch (Exception $e) {
    ...
    $vehiclePath = "vehicle?id=eq.{$tripData['vehicle_id']}";
        ...
        // Vehicle table enforces a status CHECK constraint: available, assigned, out_for_delivery, unavailable
        $vehicleStatusToSet = 'assigned'; // when a trip is created and vehicle assigned
    ...
    $driverPath = "drivers?id=eq.{$tripData['driver_id']}";
        ...
        // Driver table enforces a status CHECK constraint: available, assigned, out_for_delivery, unavailable
        $driverStatusToSet = 'assigned'; // driver assigned to a trip
