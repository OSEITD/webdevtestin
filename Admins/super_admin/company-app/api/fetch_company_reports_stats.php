...
// IMPORTANT: display_errors must stay OFF – any HTML output before JSON breaks the response
    ...
    public function handleRequest() {
        try {
            ...
            try {
                ...
                try {
                        $outletsResult = $this->supabase->getRecord("outlets?company_id=eq.{$companyId}&deleted_at=is.null&select=*");
                        $driversResult = $this->supabase->getRecord("drivers?company_id=eq.{$companyId}&deleted_at=is.null&select=*");
            ...
            // Calculate Delivery Status Breakdown (Dynamic)
            $delivery_status_counts = [];
            foreach ($parcelsArr as $p) {
                $status = isset($p['status']) && $p['status'] !== '' ? trim((string)$p['status']) : 'Unknown';
                // Capitalize properly
                $status = ucwords(strtolower($status));
                if (!isset($delivery_status_counts[$status])) {
                    $delivery_status_counts[$status] = 0;
                }
                $delivery_status_counts[$status]++;
            }

            // Calculate Deliveries Over Time (for chart)
            $chartLabels = [];
            $chartData = [];
            try {
                $sd = $startDate ? new DateTime($startDate) : clone (new DateTime())->modify('-6 days');
                $ed = $endDate ? new DateTime($endDate) : new DateTime();
                $diffDays = $sd->diff($ed)->days;
                
                if ($diffDays <= 31) {
                    $curr = clone $sd;
                    while ($curr <= $ed) {
                        $dateStr = $curr->format('Y-m-d');
                        $chartLabels[] = '"' . $curr->format('D, M j') . '"'; // Example: "Mon, Oct 14"
                        $chartData[$dateStr] = 0;
                        $curr->modify('+1 day');
                    }
                    foreach ($parcelsArr as $p) {
                        $created = isset($p['created_at']) ? substr($p['created_at'], 0, 10) : null;
                        if ($created && isset($chartData[$created])) $chartData[$created]++;
                    }
                } else {
                    $curr = clone $sd;
                    ...
                    while ($curr <= $endMonth) {
                        ...
                        $chartLabels[] = '"' . $curr->format('M Y') . '"'; // Example: "Oct 2023"
                        $chartData[$monthStr] = 0;
                        $curr->modify('+1 month');
                    }
                    foreach ($parcelsArr as $p) {
                        $created = isset($p['created_at']) ? substr($p['created_at'], 0, 7) : null;
                        if ($created && isset($chartData[$created])) $chartData[$created]++;
                    }
                }
            } catch (Exception $e) {
                // Ignore date parsing errors and fallback to empty
                $chartLabels = []; 
                $chartData = [];
            }
            
            $deliveries_by_day = [
                'labels' => str_replace('"', '', $chartLabels), // Remove quotes for array
            ...
            // Get commission_rate from company record
            try {
                $companyData = $this->supabase->getCompany($_SESSION['id'], $_SESSION['access_token'] ?? null);
                if (is_array($companyData) && isset($companyData[0]['commission_rate'])) {
                    $commission_rate = floatval($companyData[0]['commission_rate']);
                } elseif (is_object($companyData) && isset($companyData->data) && is_array($companyData->data) && isset($companyData->data[0]['commission_rate'])) {
                    $commission_rate = floatval($companyData->data[0]['commission_rate']);
                }
            } catch (Exception $ce) {
                error_log('fetch_company_reports_stats: commission_rate fetch failed: ' . $ce->getMessage());
            }
            ...
            try {
                ...
                // Normalize the response - extract from object wrapper if needed
                $txData = [];
                if (is_object($txResponse) && isset($txResponse->data)) {
                    $txData = is_array($txResponse->data) ? $txResponse->data : [$txResponse->data];
                } elseif (is_array($txResponse)) {
                    $txData = $txResponse;
                }
                
                    // Get the 10 most recent transactions, ignoring date ranges
                    $filtered_txs = [];
                    foreach ($txData as $tx) {
                        if (count($filtered_txs) >= 10) break;
                    ...
                    error_log('fetch_company_reports_stats: found ' . $total_transactions . ' recent transactions');
                    
                    // Calculate total commission and build transactions array
                ...
                // silently fail for transactions - they're supplementary data
                error_log('fetch_company_reports_stats: transaction fetch failed: ' . $e->getMessage());
            }
            
            // Add commission to each revenue_breakdown item
            foreach ($revenue_breakdown as $idx => $rb) {
                $rb_revenue = floatval($rb['revenue'] ?? 0);
                $rb_commission = round(($rb_revenue * $commission_rate / 100), 2);
                $revenue_breakdown[$idx]['commission_rate'] = $commission_rate;
                $revenue_breakdown[$idx]['commission'] = $rb_commission;
            }
