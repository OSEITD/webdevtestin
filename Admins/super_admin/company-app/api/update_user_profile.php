...
if (isset($update['email'])) {
    ...
    $checkUrl = $supabaseUrl . '/rest/v1/profiles?email=eq.' . urlencode($update['email']) . 
                '&id=neq.' . urlencode($userId) . '&select=id';
    ...
    $existingEmail = json_decode($checkResponse, true);
    curl_close($ch);
...
if (isset($update['phone'])) {
    ...
    $checkUrl = $supabaseUrl . '/rest/v1/profiles?phone=eq.' . urlencode($update['phone']) . 
                '&id=neq.' . urlencode($userId) . '&select=id';
    ...
    $checkResponse = curl_exec($ch);
    $existingPhone = json_decode($checkResponse, true);
    curl_close($ch);
    
    if (!empty($existingPhone) && !isset($existingPhone['error'])) {
        http_response_code(409);
        echo json_encode([
            'success' => false,
            'error' => 'This phone number is already in use by another account',
            'errors' => ['phone' => 'This phone number is already in use by another account.']
        ]);
        exit;
    }
}
