...
$allowed_statuses = ['available', 'assigned', 'out_for_delivery', 'unavailable'];
...
try {
    ...
    $checkUrl = $supabaseUrl . '/rest/v1/drivers?driver_email=eq.' . urlencode($data['driver_email']) . '&deleted_at=is.null&select=id' .
    ...
    if (!empty($existingDrivers)) {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'A driver with this email already exists in your company',
            'errors' => ['driver_email' => 'A driver with this email already exists in your company.']
        ]);
        exit;
    }

    // Now check if a driver with this phone number already exists
    $phoneCheckUrl = $supabaseUrl . '/rest/v1/drivers?driver_phone=eq.' . urlencode($data['driver_phone']) . '&deleted_at=is.null&select=id' .
                     '&company_id=eq.' . $_SESSION['id'];
    
    $ch = curl_init($phoneCheckUrl);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        "apikey: $supabaseKey",
        "Authorization: Bearer $supabaseKey"
    ...
    if (!empty($existingPhoneDrivers)) {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'A driver with this phone number already exists in your company',
            'errors' => ['driver_phone' => 'This phone number is already registered to another driver in your company.']
        ]);
        exit;
    }
    ...
    // Update profile with company_id and email
    ...
    error_log("Updating driver profile at: " . $profileUrl);
