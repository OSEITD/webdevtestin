...
try {
    // Fetch companies data with revenue
    $companiesData = callSupabase('companies?select=id,company_name,revenue,commission_rate,currency,status');
    if (is_array($companiesData)) {
        ...
        foreach ($companiesData as $company) {
            if ($company['status'] === 'active') {
                // Revenue is stored as a number
                $totalRevenue += floatval($company['revenue'] ?? 0);
                // Commission rate is percentage, multiply by revenue to get commission amount
                $commissionAmount = floatval($company['revenue'] ?? 0) * ($company['commission_rate'] ?? 0) / 100;
                $platformCommissions += intval($commissionAmount);
            }
        }
    }
    ...
    // Get delivery counts
    $deliveredParcels = callSupabase('parcels?select=id&status=eq.delivered');
...
if (!empty($companiesData)) {
    foreach ($companiesData as $company) {
        if ($company['status'] === 'active') {
            $companyEarnings[] = [
                ...
                'id' => $company['id'],
                'earnings_major_units' => $company['revenue'] ?? 0,
    ...
    usort($companyEarnings, fn($a, $b) => $b['earnings_major_units'] - $a['earnings_major_units']);
        ...
        <main class="main-content">
            ...
            <div class="stat-card revenue-card animate-fade-in">
                ...
                <div class="stat-content">
                    ...
                    <p class="value" data-stat="total_revenue" <?php 
                        $minorTotalRevenue = intval(round($totalRevenue * pow(10, getCurrencyDecimals(getEffectiveCurrency()))));
                        echo 'data-currency-value="' . $minorTotalRevenue . '" data-currency="' . htmlspecialchars(getEffectiveCurrency()) . '"'; 
                    ?>>
                        <?php echo formatCurrency($totalRevenue, getEffectiveCurrency()); ?>
                    </p>
            ...
            <div class="quick-actions-section animate-fade-in">
                ...
                <div class="quick-action-buttons">
                    ...
                    <a href="help.php" class="action-btn secondary">
                ...
                <div class="earnings-summary-grid">
                    <div class="earnings-summary-card">
                        ...
                        <div class="stat-content">
                            ...
                            <p class="value" <?php 
                                $minorCommissions = intval(round($platformCommissions * pow(10, getCurrencyDecimals(getEffectiveCurrency()))));
                                echo 'data-currency-value="' . $minorCommissions . '" data-currency="' . htmlspecialchars(getEffectiveCurrency()) . '"'; 
                            ?>>
                                <?php echo formatCurrency($platformCommissions, getEffectiveCurrency()); ?>
                            </p>
                    ...
                    <div class="earnings-summary-card">
                        ...
                        <div class="stat-content">
                            ...
                            <p class="value" <?php 
                                ...
                                $minorAvgRevenue = intval(round($avgRevenue * pow(10, getCurrencyDecimals(getEffectiveCurrency()))));
                                echo 'data-currency-value="' . $minorAvgRevenue . '" data-currency="' . htmlspecialchars(getEffectiveCurrency()) . '"'; 
                            ?>>
                                <?php echo formatCurrency($avgRevenue, getEffectiveCurrency()); ?>
                            </p>
                        ...
                        <?php foreach ($topCompanies as $index => $company): ?>
                            <div class="company-earnings-item">
                                <div class="company-info">
                                    ...
                                    <p class="revenue" <?php 
                                        $minorCompanyRevenue = intval(round($company['earnings_major_units'] * pow(10, getCurrencyDecimals($company['currency'] ?? getEffectiveCurrency()))));
                                        echo getCurrencyDataAttributes($minorCompanyRevenue, $company['currency'] ?? null); 
                                    ?>>
                                        <?php echo formatCurrency($company['earnings_major_units'], $company['currency'] ?? null); ?>
                                    </p>
                                </div>
                                <div class="progress-bar-container">
                                    <div class="progress-bar" style="--progress-width: <?php echo ($totalRevenue > 0) ? (($company['earnings_major_units'] / $totalRevenue) * 100) : 0; ?>%;">
                                        <span class="progress-label"><?php echo ($totalRevenue > 0) ? round(($company['earnings_major_units'] / $totalRevenue) * 100) : 0; ?>%</span>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
    ...
    async function updateDashboardStats() {
        try {
            ...
            if (data.success) {
                ...
                if (revenueElement && data.revenue?.total !== undefined) {
                    const minorTotal = Math.round(Number(data.revenue.total) * Math.pow(10, currency.decimals || 2));
                    revenueElement.setAttribute('data-currency-value', minorTotal);
                    revenueElement.setAttribute('data-currency', currency.code);
                    if (window.currencyManager) {
                        window.currencyManager.updateElementCurrency(revenueElement, minorTotal, currency.code);
                    }
                }
                ...
                if (commissionsCards.length > 0 && data.revenue?.commissions !== undefined) {
                    const minorCommissions = Math.round(Number(data.revenue.commissions) * Math.pow(10, currency.decimals || 2));
                    commissionsCards[0].setAttribute('data-currency-value', minorCommissions);
                    commissionsCards[0].setAttribute('data-currency', currency.code);
                    if (window.currencyManager) {
                        window.currencyManager.updateElementCurrency(commissionsCards[0], minorCommissions, currency.code);
                    }
                }
                ...
                if (avgRevenueCards.length > 1 && stats.total_companies > 0) {
                    ...
                    const minorAvg = Math.round(Number(avgRevenue) * Math.pow(10, currency.decimals || 2));
                    avgRevenueCards[1].setAttribute('data-currency-value', minorAvg);
                    avgRevenueCards[1].setAttribute('data-currency', currency.code);
                    if (window.currencyManager) {
                        window.currencyManager.updateElementCurrency(avgRevenueCards[1], minorAvg, currency.code);
                    }
                }
                // Update company list
    ...
    companies.forEach(company => {
        ...
        div.innerHTML = `
            <div class="company-info">
                ...
                <p class="revenue" data-currency-value="${Math.round(Number(company.earnings) * Math.pow(10, (company.currency_decimals || currency.decimals || 2)))}" data-currency="${company.currency || currency.code}">${(company.currency_symbol || currency.symbol)}${Number(company.earnings).toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                })} ${company.currency || currency.code}</p>
            </div>
            <div class="progress-bar-container">
