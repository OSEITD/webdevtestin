...
try {
    ...
    // Step 2b: Check if phone already exists
    $existingPhone = callSupabaseWithServiceKey('profiles?phone=eq.' . urlencode($adminData['phone']) . '&select=id', 'GET');
    ...
    $existingEmail = callSupabaseWithServiceKey('profiles?email=eq.' . urlencode($adminData['email']) . '&select=id', 'GET');
