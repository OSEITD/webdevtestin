...
try {
    ...
    $existingPhone = callSupabaseWithServiceKey('profiles?phone=eq.' . urlencode($customerData['phone']) . '&select=id', 'GET');
    ...
    $existingEmail = callSupabaseWithServiceKey('profiles?email=eq.' . urlencode($customerData['email']) . '&select=id', 'GET');
    ...
    try {
        ...
        $msg = $e->getMessage();
        if (strpos($msg, 'email_exists') !== false || strpos($msg, 'already been registered') !== false || strpos($msg, 'already registered') !== false) {
            http_response_code(409);
            echo json_encode([
                'success' => false,
                'error' => 'A user with this email already exists',
    ...
    if (!$userResult || !isset($userResult['id'])) {
        if (isset($userResult['message']) && (strpos($userResult['message'], 'already registered') !== false || strpos($userResult['message'], 'email_exists') !== false)) {
            http_response_code(409);
            echo json_encode([
                'success' => false,
                'error' => 'A user with this email already exists',
