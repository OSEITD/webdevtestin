...
try {
    ...
    $defaultCurrency = 'USD';
    if (is_array($settings) && !empty($settings[0]) && !empty($settings[0]['global_currency'])) {
        $defaultCurrency = strtoupper(substr($settings[0]['global_currency'], 0, 3));
    }
    ...
    $companiesData = callSupabase('companies?select=id,company_name,revenue,commission_rate,currency,status&order=revenue.desc');
    $totalRevenue = 0;
    ...
    if (is_array($companiesData)) {
        ...
        foreach ($companiesData as $company) {
            if ($company['status'] === 'active') {
                $revenue = floatval($company['revenue'] ?? 0);
                $commissionRate = floatval($company['commission_rate'] ?? 0);
