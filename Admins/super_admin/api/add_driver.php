...
try {
    ...
    // Step 2: Check if phone already exists within same company
    $existingPhone = callSupabaseWithServiceKey(
        'drivers?phone=eq.' . urlencode($driverData['driver_phone']) . '&company_id=eq.' . urlencode($driverData['company_id']) . '&select=id',
        'GET'
    );
    if (!empty($existingPhone)) {
        ...
        echo json_encode([
            ...
            'errors' => ['driver_phone' => 'This phone number is already registered to another driver in this company.']
        ]);
        exit;
    }

    // Step 2b: Check if email already exists
    $existingEmail = callSupabaseWithServiceKey('profiles?email=eq.' . urlencode($driverData['driver_email']) . '&select=id', 'GET');
    if (!empty($existingEmail)) {
        http_response_code(409);
        echo json_encode([
            'success' => false,
            'error' => 'A driver with this email already exists',
            'errors' => ['driver_email' => 'This email is already registered. Please use a different email.']
        ]);
        exit;
    }
    ...
    if (!$profileResult || !isset($profileResult['id'])) {
        if (isset($profileResult['message']) && (strpos($profileResult['message'], 'already registered') !== false || strpos($profileResult['message'], 'email_exists') !== false)) {
            http_response_code(409);
            echo json_encode([
                'success' => false,
                'error' => 'A driver with this email already exists',
