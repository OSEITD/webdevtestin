 ...
 * 
 * Usage:
 *   ErrorHandler::handleException($e, 'fetch_companies.php');
...
class ErrorHandler {
    ...
    public static function handleException($e, $endpoint, $defaultHttpCode = 500) {
        ...
        // Determine if this error message is safe to show to the user
    ...
    public static function requireAuth($endpoint) {
        if (!isset($_SESSION['user_id']) || empty($_SESSION['user_id']) || !isset($_SESSION['access_token'])) {
            self::logError(
                "Unauthorized access attempt (missing session data)",
                $endpoint,
                [
                    'has_id' => isset($_SESSION['user_id']),
